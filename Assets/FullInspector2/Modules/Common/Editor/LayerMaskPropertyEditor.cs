...
namespace FullInspector.Internal {
    ...
    public class LayerMaskEditor : PropertyEditor<LayerMask> {
        ...
        public static LayerMask LayerMaskField(Rect region, GUIContent label, LayerMask selected) {
            // NOTE: Sign bit is treated as "everything" by Unity, so it cannot
            //       be a layer. That's why we iterate to 31, *not* 32.
            ...
            // Map the current LayerMask value from its LayerMask assosciation
            // into our assosciation for layers.
            int mappedValue = 0;
            for (int i = 0; i < 32; ++i) {
            ...
            // NOTE: We can avoid GC by allocating and pooling arrays of the
            //       required size. We could run a population count on mask to
            //       figure out the array size to use.
