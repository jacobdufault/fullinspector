...
namespace FullInspector {
    /// <summary>
    /// The cached height metadata is used to optimize CPU/execution time for
    /// some memory so that we don't have to recompute heights multiple times
    /// over. In essence, if we call GetElementHeight from Edit, then
    /// GetElementHeight will short-circuit and just return a cached height.
    /// </summary>
    ...
    public static class PropertyEditorExtensions {
        ...
        private static T DoEditSlow<T>(IPropertyEditorEditAPI api, Rect region, GUIContent label, T element, fiGraphMetadata metadata) {
            ...
            if (dropdown.ShouldDisplayDropdownArrow) {
                // The foldout is hidden. Just draw the collapsed arrow and
                // return.
                if (dropdown.IsActive == false) {
                    // Some editors don't always supply a label (for example, the
                    // collection editors), so instead of showing nothing we'll
                    // show the current ToString() value
                    ...
                    // note: we don't use dropdown.IsActive = ... because we can
                    //       be animating, and doing that direct-assign toggle
                    // will cause the animation to flip-flop infinitely
                ...
                // Okay, we're going to be rendering the content of the body. We
                // want to figure out how to actually render the dropdown arrow.
                // We do it in one of two ways:
                // 1. Draw using EditorGUI.Foldout with the full label
                // 2. Draw just the foldout and indent the rect if we're not in
                //    hierarchy mode

                // We have a couple of special cases to handle:
                // 1. We don't have a label
                // 2. The editor has non-standard label rendering In these
                // scenarioes, we will just draw the foldout arrow by itself and
                // indent the contents of the region.
            ...
            // End the cull zone. This should have been started inside of
            // GetElementHeight, but if for some reason GetElementHeight was
            // never called, this will be harmless if not currently in a
            // cull-zone.
        ...
        /// <summary>
        /// Fetches the dropdown metadata instance that should be used. This
        ...
        /// <summary>
        /// Returns a header that should be used for the foldout. An item is
        /// displayed within a foldout when this property editor reaches a
        /// certain height.
        /// </summary>
        ...
        /// <summary>
        /// This method makes it easy to use a typical property editor as a
