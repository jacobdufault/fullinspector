...
namespace FullInspector.Internal {
    ...
    /// </summary>
    // TODO: Should the SetDirty calls in fiISerializedObjectUtility be moved into this class? I think it might make sense to do it,
    //       because this is the "editor" serialization integration.
    // TODO: There appears to be a bug in Unity. If we save the state of a prefab when it is not being actively inspected,
    //       prefab changes will *not* get propagated to the prefab instances, despite the prefab having received those changes.
    //       The changes get propagated after entering play-mode, though.
    // NOTE: With the current approach of deferring serialization for the actively inspected object, if the user compiles and edits
    //       data while compiling but a serialization does *not* run after the edit and before the script reload, the edited changes
    //       will be lost.
    // TODO: Figure out if we can find a good work-around for the above note. We could just serialize constantly if we are compiling,
    //       but that causes recompiles to take a lot longer.
    public static class fiSerializationManager {
        ...
        private static void OnEditorUpdate() {
            ...
            // Deserialize any inspected objects that have not been restored yet. This happens when
            // adding a new component since Unity does not call ISerializationCallbackReceiver.OnDeserialized.
        ...
        private static void DoSerialize(ISerializedObject obj) {
            ...
            if (forceSerialize == false && obj is UnityObject) {
                ...
                // throw an exception if we are currently on a non-Unity
                // thread.
